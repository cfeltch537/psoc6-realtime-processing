...
bool static requestImuIndication    =   false;
bool static requestImuNotification    =   false;
            ...
            if(writeReqParameter->handleValPair.attrHandle == CY_BLE_DATAFRAMES_REALTIME_CCCD_DESC_HANDLE){
                ...
                /* Store data in the database */
                 ...
                 if(gattErr != CY_BLE_GATT_ERR_NONE)
                 {
                    ...
                    requestImuNotification = writeReqParameter->handleValPair.value.val[0] & 0x01;
                                 ...
                                 uint8_t* value, uint16_t len)
{
    ...
    /* Variable used to store the return values of BLE APIs */
    cy_en_ble_api_result_t bleApiResult;
    cy_en_ble_gatt_err_code_t gattApiResult;
    ...
    gattApiResult = Cy_BLE_GATTS_WriteAttributeValueLocal(&handleValuePair); 
    /* Check if the operation has been successful */
    if(gattApiResult != CY_BLE_GATT_ERR_NONE)
    {
        Task_DebugPrintf("Failure! : BLE - Writting GATT Attribute. "\
                    "Error Code:", gattApiResult);
    }
